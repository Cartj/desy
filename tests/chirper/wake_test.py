...
filename=drive+'TEST_DECHIRPER/ASTRA/XFEL01/WakeSLAC0.1600.001'
...
plt.figure(1)
plt.plot(P[:,2]*1e6,Pz*1e-6,'.',(w1[:,0]+25e-3)*1e-3*1e6,w1[:,1]*1e12/2*1e-6,I00[:,0]*1e6,-I00[:,1]*koef*1e-6)
plt.figure(2)
koef=np.min(Px)/np.max(I00[:,1]);
plt.plot(P[:,2]*1e6,Px*1e-6/x0,'.',(w1[:,0]+25e-3)*1e-3*1e6,w1[:,3]*1e15/2*1e-6,I00[:,0]*1e6,-I00[:,1]*koef*1e-6)
plt.figure(3)
koef=np.min(Py)/np.max(I00[:,1]);
plt.plot(P[:,2]*1e6,Py*1e-6/y0,'.',(w1[:,0]+25e-3)*1e-3*1e6,w1[:,3]*1e15*1e-6,I00[:,0]*1e6,-I00[:,1]*koef*1e-6)
plt.show()