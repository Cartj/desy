...
def plot_stats(idx='', base='', root='/FEL/', dump_ascii=False, columns=['all']):

    xdb = Xdb(index_file=idx, mode='r')
    
    fig = figure()    
    fig.suptitle('Pulse power')
    ...
    p_med = np.array(xdb.file[root + base + '/pulse_power_med'])
    p_mean = np.array(xdb.file[root + base + '/pulse_power_mean'])
    p_std = np.array(xdb.file[root + base + '/pulse_power_std'])
    ...
    E_onaxis_med = np.array(xdb.file[root + base + '/field_onaxis_med'])
    ...
    if dump_ascii:
	    ...
	    for i in xrange(len(t)):
	    	f.write(str(t[i]) + '\t' + str(p_mean[i]/ 1.e9) + '\t' + str(E_onaxis_med[i]) + '\n')	
	    f.close()
    ...
    if dump_ascii:
	    f=open('spec.txt','w')
	    for i in xrange(len(t)):
	    	f.write(str(t[i]) + '\t' + str(s_mean[i]*Y*X) + '\n')	
	    f.close()
...
def extract(path, ran, h5_file = None):
    ...
    for run_id in ran:
	    ...
	    E_onaxis = np.zeros_like(slices[:,0,0])
            for i in xrange(len(P)):
                ...
                E[i] = sum( slices[i,:,:])
        	E_onaxis[i] = slices[i, slices.shape[1]/2,slices.shape[2]/2]
    ...
    if rank == 0:
    	...
    	f_mean, f_std, f_med, f_worst, f_imed, f_iworst = stats(fields_mean)
	ff_mean, ff_std, ff_med, ff_worst, ff_imed, ff_iworst = stats(fields_onaxis)
	    ...
	    params['field_onaxis_med'] = fields_onaxis[ff_imed]
...
parser.add_argument('--dump_ascii', help='dump ascii files for median field intensities', action='store_true')
parser.add_argument('--dest', help='dest')
parser.add_argument('--columns', help='columns', default='all')
...
if args.plot:
    plot_stats(idx=args.file, base=args.path, root='/', dump_ascii=args.dump_ascii)
